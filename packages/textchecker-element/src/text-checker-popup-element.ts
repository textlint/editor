...
${items.map(item => {
            ...
            if (item.message) {
                if (item.onClick) {
                    return html`<li @click=${clickHandler} class="popup-listItem" style="--padding: ${itemPadding}px;"><p class="popup-listItem-message">${item.message}</p>
<p class="popup-listItem-content">${item.icon}${item.label}</p></li>`
                } else {
                    return html`<li @click=${clickHandler} class="popup-listItem" style="--padding: ${itemPadding}px; padding-bottom: 0;"><p class="popup-listItem-message">${item.message}</p>
<p class="popup-listItem-content">${item.icon}${item.label}</p></li>`

                }

            }
            return html`<li @click=${clickHandler} class="popup-listItem" style="--padding: ${itemPadding}px;"><p class="popup-listItem-content">${item.icon}${item.label}</p></li>`
        })}
</ul>`;
        // https://www.figma.com/file/9kRm0Cr869zbdACytRE74R/textlint-editor?node-id=4%3A2
        const style = `position: fixed; z-index: 2147483644; width: ${rect.width}px; min-width: 300px; top: ${rect.top}px; left: ${rect.left - itemPadding}px;`;
